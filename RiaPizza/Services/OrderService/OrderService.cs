...
namespace RiaPizza.Services.OrderService
{
    public class OrderService : IOrderService
    {
        ...
        {
            var orders = await _context.Orders.Where(s => s.UserId == id && s.OrderDateTime.Year == DateTime.Now.Year && s.OrderDateTime.Month == DateTime.Now.Month && s.OrderDateTime.Day == DateTime.Now.Day).Include(s => s.OrderItems).ThenInclude(s => s.Dish).ToListAsync();
